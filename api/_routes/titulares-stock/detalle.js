...
async function actualizarTitular(req, res, id) {
  try {
    ...
    if (error) {
      return res.status(500).json({ error: formatearErrorDb(error, 'actualizando titular de stock') })
    }
...
async function eliminarTitular(req, res, id) {
  try {
    ...
    if (error) {
      return res.status(500).json({ error: formatearErrorDb(error, 'eliminando titular de stock') })
    }

...
export default function FinancierasList() {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      const res = await api.get(`/api/financieras?${params.toString()}`)
      setFinancieras(res.data)
  ...
  useEffect(() => {
    if (location.state?.creado) {
      setOk('Financiera creada correctamente.')
  ...
  async function confirmarEliminar() {
    ...
    setErrorEliminar('')
    try {
      await api.delete(`/api/financieras/detalle?id=${aEliminar.id}`)
      setAEliminar(null)
      cargar()
    } catch (err) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          type="button"
          onClick={() => navigate('/finanzas/financieras/nuevo')}
      ...
      <div className="bg-white border border-slate-400 rounded-xl p-4 mb-4">
        <form onSubmit={handleBuscar} className="flex flex-wrap items-end gap-3">
          <div className="flex-1 min-w-[240px]">
            <label className="text-xs font-medium text-slate-600 mb-1 block">Buscar Financiera</label>
            <input
              value={busquedaInput}
      ...
      <div className="bg-white border border-slate-400 rounded-xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {!cargando && financieras.length === 0 && (
              <tr><td colSpan={3} className="px-4 py-8 text-center text-slate-600">No hay financieras registradas.</td></tr>
            )}
            {!cargando &&
              financieras.map((f) => (
                <tr key={f.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-800">
                    <button type="button" onClick={() => navigate(`/finanzas/financieras/${f.id}/editar`)} className="hover:text-blue-600 hover:underline">
                      {f.nombre}
                    </button>
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${f.activo ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-600'}`}>
                      {f.activo ? 'Activa' : 'Inactiva'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 text-slate-600">
                      <button type="button" onClick={() => navigate(`/finanzas/financieras/${f.id}/editar`)} className="hover:text-amber-600" title="Editar">
                        <Pencil size={16} />
                      </button>
                      <button type="button" onClick={() => setAEliminar(f)} className="hover:text-red-600" title="Eliminar">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </td>

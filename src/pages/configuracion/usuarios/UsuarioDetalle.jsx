    ...
    <div>
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500">Información básica del usuario</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => navigate(`/configuracion/usuarios/${id}/editar`)} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg px-3 py-1.5">
            <Pencil size={14} /> Editar Usuario
          </button>
          <button onClick={() => navigate('/configuracion/usuarios')} className="flex items-center gap-1.5 text-sm font-medium text-slate-600 border border-slate-200 rounded-lg px-3 py-1.5 hover:bg-slate-50">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-6 max-w-lg">
        ...
        <dl className="space-y-3 text-sm">
          <Fila label="Estado de la Cuenta">
            <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${usuario.activo ? 'bg-green-50 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
